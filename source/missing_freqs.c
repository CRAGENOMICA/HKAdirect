...
float cs(int i, int j, int k, int l, int nx, int ny, int nxy,float theta){
	...
	float sum=0.0;
	float b1,b2,b3;
	first_factor = exp(lnbinomial(nx-nxy,l-j)+lnbinomial(ny - nxy,k - i)- lnmultinomial(nx+ny-nxy,l,k-l));
	if ((l-j<0)||(nx-nxy-l+j<0)||(k-i<0)||(ny-nxy-k+i<0)||(nx+ny-nxy-k<0)) return 0;
	lb = max(i-nxy,l-j);
	ub = min3(i,nx-nxy,k-j);
	for(kx=lb;kx<=ub;kx++){
		b1=lnbinomial(nxy,i-kx);
		b2=lnbinomial(nx-nxy+j-l,kx+j-l);
		b3=lnbinomial(k-kx,j);
		sum+=exp(b1+b2+b3); 
	}
...
float ce(int i, int j, int k, int l, int nx, int ny, int nxy,float theta){
	...
	first_factor = exp(lnbinomial(nx-nxy,l-j)+lnbinomial(ny - nxy,k - i)- lnmultinomial(nx+ny-nxy,k,l)); 
	/*fprintf(stdout,"i,j,k,l:%d,%d,%d,%d first factor:%g\n",i,j,k,l,first_factor);*/
	lb = max3(0,i-nxy,k-ny+j);
	ub = min(i,nx-nxy+j-l);
	/*fprintf(stdout,"lb=%d,ub=%d\n",lb,ub);*/
	for(kx=lb;kx<=ub;kx++){
		b1=lnbinomial(nxy,i-kx);
		b2=lnbinomial(nx-nxy+j-l,kx);
		b3=lnbinomial(ny-k+kx,j);
...
float pc(int j,int *r,int n){
	...
	int *conf;
	float s;
    conf=calloc(CONFSIZE,sizeof(int)); 
	store_configurations(n, j , conf,&nconf);
    s = hellman_sum(conf, nconf, n, r);   
    return s;
}
int check_conf(int j, int n,int* conf){
